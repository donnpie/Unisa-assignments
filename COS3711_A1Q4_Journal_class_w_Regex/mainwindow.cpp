    ...
    ui(new Ui::MainWindow)
{
    ...
    QRegExpValidator *authorVal = new QRegExpValidator(authorRx, this);  //Why is this not working?
    ...
    */
    ...
    QRegularExpressionValidator *authorVal = new QRegularExpressionValidator(authorRx, ui->gridLayoutWidget);
    QRegularExpressionValidator *titleval = new QRegularExpressionValidator(titleRx, ui->gridLayoutWidget);
    QRegularExpressionValidator *journalVal = new QRegularExpressionValidator(journalRx, ui->gridLayoutWidget);
    QRegularExpressionValidator *pagesVal = new QRegularExpressionValidator(pagesRx, ui->lineEditPages);
    ui->lineEditAuthor->setValidator(authorVal);
    ui->lineEditTitle->setValidator(titleval);
    ui->lineEditJournal->setValidator(journalVal);
    ui->lineEditPages->setValidator(pagesVal);
    ...
    //QRegExpValidator *tempV = new QRegExpValidator;
    //tempV = ui->lineEditPages->validator();

    qDebug() << "QRegExpValidator pointer address: " << &pagesVal;
    qDebug() << "QRegExpValidator target address: " << pagesVal;
    qDebug() << "UI QRegExpValidator target address: " << ui->lineEditPages->validator();
    if (ui->lineEditPages->validator() == nullptr)
    {
        QMessageBox::warning(nullptr, "Error", "The pages validator was not set");
    }
    ...
    //Debug code for validator
    //connect(ui->lineEditPages->validator(), SIGNAL(changed), this, SLOT(validatorChanged()));
...
void MainWindow::validatorChanged()
{
    QMessageBox::warning(nullptr, "Error", "The pages validator was changed");
}
...
{
    //Highlight records older than OLD_REFS and younger than RECENT_REFS
